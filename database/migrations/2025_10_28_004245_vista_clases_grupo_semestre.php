...
return new class extends Migration {
    ...
    SELECT
        ...
        COUNT(DISTINCT CASE
            WHEN `palumno`.`id` IS NOT NULL AND `calumno`.`id` IS NOT NULL
            THEN `cal`.`idAlumno`
            ELSE NULL
        END) AS `alumnosInscritos`
    ...
    LEFT JOIN `{$dbName}`.`alumno` `alum` ON `cal`.`idAlumno` = `alum`.`id`
    LEFT JOIN `{$dbName}`.`persona` `palumno` ON `alum`.`idPersona` = `palumno`.`id` AND `palumno`.`deleted_at` IS NULL
    LEFT JOIN `{$dbName}`.`cuenta` `calumno` ON `palumno`.`idCuenta` = `calumno`.`id` AND `calumno`.`deleted_at` IS NULL
    GROUP BY
